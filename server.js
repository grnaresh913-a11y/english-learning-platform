...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    await pool.query('INSERT INTO gamification (user_id, points, badges, streak) VALUES ($1, 0, $2, 0)', [user.id, '[]']);
...
app.post('/api/assessments', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query('INSERT INTO assessments (user_id, concept_id, score, feedback) VALUES ($1, $2, $3, $4) RETURNING *', [req.user.id, concept_id, score, feedback]);
...
app.post('/api/progress', authenticateToken, async (req, res) => {
  try {
    ...
    let progress = await pool.query('SELECT * FROM progress WHERE user_id = $1 AND concept_id = $2', [req.user.id, concept_id]);
    if (progress.rows.length === 0) {
      const result = await pool.query('INSERT INTO progress (user_id, concept_id, repetitions, ease_factor, next_review, status) VALUES ($1, $2, 1, 2.5, NOW() + INTERVAL \'1 day\', \'learning\') RETURNING *', [req.user.id, concept_id]);
    ...
    const result = await pool.query('UPDATE progress SET repetitions = repetitions + 1, ease_factor = $1, next_review = NOW() + ($2 || \' days\')::INTERVAL WHERE user_id = $3 AND concept_id = $4 RETURNING *', [newEase, interval, req.user.id, concept_id]);
    res.json(result.rows[0]);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.post('/api/gamification/points', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query('UPDATE gamification SET points = points + $1 WHERE user_id = $2 RETURNING *', [points, req.user.id]);
    res.json(result.rows[0]);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get('/api/leaderboard', async (req, res) => {
  try {
    const result = await pool.query('SELECT u.id, u.name, g.points, g.streak FROM users u JOIN gamification g ON u.id = g.user_id ORDER BY g.points DESC LIMIT 20');
    res.json(result.rows);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.post('/api/tutoring/book', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query('INSERT INTO tutoring_sessions (user_id, tutor_id, session_date, duration) VALUES ($1, $2, $3, $4) RETURNING *', [req.user.id, tutor_id, session_date, duration]);
    res.status(201).json(result.rows[0]);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.post('/api/audio/save', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query('INSERT INTO audio_recordings (user_id, module, audio_url, transcript, pronunciation_score) VALUES ($1, $2, $3, $4, $5) RETURNING *', [req.user.id, module, audio_url, transcript, pronunciation_score]);
    res.status(201).json(result.rows[0]);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.post('/api/grammar/check', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query('INSERT INTO grammar_checks (user_id, text_input, errors, corrections, score) VALUES ($1, $2, $3, $4, 80) RETURNING *', [req.user.id, text_input, '[]', '[]']);
    res.status(201).json(result.rows[0]);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.post('/api/plagiarism/check', authenticateToken, async (req, res) => {
  try {
    const { text_input } = req.body;
    const result = await pool.query('INSERT INTO plagiarism_checks (user_id, text_input, plagiarism_score, matched_sources) VALUES ($1, $2, 0.5, $3) RETURNING *', [req.user.id, text_input, '[]']);
    res.status(201).json(result.rows[0]);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.post('/api/analytics', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query('INSERT INTO analytics (user_id, module, time_spent, concepts_learned, accuracy_score) VALUES ($1, $2, $3, $4, $5) RETURNING *', [req.user.id, module, time_spent, concepts_learned, accuracy_score]);
    res.status(201).json(result.rows[0]);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
